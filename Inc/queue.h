...
typedef enum {
	QUEUE_MODE_REJECT = 0,    // Rifiuta l'inserimento se la coda è piena (Default)
	QUEUE_MODE_OVERWRITE = 1  // Sovrascrive il dato più vecchio se la coda è piena
} queue_mode_t;
...
struct queue_s
{
	...
	uint8_t* items;
	queue_mode_t mode;// Operating mode (Reject or Overwrite)
};
